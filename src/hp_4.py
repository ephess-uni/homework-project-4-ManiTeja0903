...
def reformat_dates(old_dates):
    ...
    dates = [datetime.strptime(oldDdate, "%Y-%m-%d").strftime('%d %b %Y') for oldDdate in old_dates]
    return dates
...
def add_date_range(values, start_date):
    ...
    dtrng = date_range(start_date, len(values))
    sa= list(zip(dtrng, values))
    return sa
...
def fees_report(infile, outfile):
    ...
    with open(outfile, 'w') as f_read:
        ...
        writer.writeheader()
        writer.writerows(header_for_opt)
